...
[English version: README.en.md](README.en.md) ／ とりあえず動かしたい方は [かんたんせつめいしょ](QUICKSTART.md) へ
...
クローン先は任意です。本書では `~/design-workflow` を前提とします。
...
初回起動時に Claude が権限マージを提案します。`yes` と回答することで自動的に設定が反映されます。手動で適用する場合は、`~/design-workflow/settings.recommended.json` の `permissions.allow` 配列を `~/.claude/settings.json` の同名キー配下に統合してください。
...
| `playwright not found` | `cd ~/design-workflow/skills/design-extractor && npm install && npx playwright install chromium` を実行 |
